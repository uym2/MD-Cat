...
void nexustree(int n,int* P,int* Suc1,int* Suc2,string* Labels,double* B,string* Support,double* T,FILE *w){
    ...
    ostringstream root;
    root<<T[r];
    fprintf(w,(Labels[r]+"[&date="+root.str()+"]").c_str());
    fprintf(w,";\n");
}
...
void nexustreeIC(int n,int* P,int* Suc1,int* Suc2,string* Labels,double* B,string* Support,double* T,double* T_min,double* T_max,FILE *w){
    ...
    rmin<<T_min[r];
    rmax<<T_max[r];
    fprintf(w,(Labels[r]+"[&date="+root.str()+",IC=\""+root.str()+"("+rmin.str()+","+rmax.str()+")\"]").c_str());
    fprintf(w,";\n");
}
...
void nexus(FILE *w,char** Labels,int n,int m){
     fprintf(w,"#NEXUS\n");
     fprintf(w,"begin taxa;\n");
     fprintf(w,"dimensions ntax=%d;\n",m-n+1);
     fprintf(w,"taxlabels\n");
     for (int i=n;i<=m;i++){
         fprintf(w,Labels[i]);
         fprintf(w,"\n");
...
void output(bool v,FILE* f,FILE* tree1,FILE* tree2,FILE* tree3,int m,int y,double rho,int* P,int* Suc1,int* Suc2,double* B,double* T,string* Labels,string* Support,double phi){
     if (v){
...
double* variance(bool w,int m,double* B,double c,int s){
