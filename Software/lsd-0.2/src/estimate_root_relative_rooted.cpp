...
double with_constraint_active_set_lambda_rate_relative(double rho,int n,int m,double br,int* & P,int* & Suc1,int* & Suc2,double* & B,double* & V,double* & T,double &lambda,double mrca){
    ...
    while (!conditions_lambda(m,ldLagrange,P,T_new,lambda_new)){
        ...
        //for (list<int>::iterator iter=active_set.begin();iter!=active_set.end();iter++) cout<<(*iter)<<" ";cout<<endl;
        //for (list<double>::iterator iter=ldLagrange.begin();iter!=ldLagrange.end();iter++) cout<<(*iter)<<" ";cout<<endl;
    }
    for (int i=0;i<n;i++) T[i]=T_new[i];
    lambda=lambda_new;
    delete[] T_new;
