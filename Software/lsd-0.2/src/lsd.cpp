...
int main( int argc, char** argv ){
    ...
    double* T_min=new double[m+1];
    double* T_max=new double[m+1];
    for (int y=1;y<=opt->nbData;y++){
            ...
            if (!opt->constraint){//LD
                if (opt->estimate_root==NULL){//keep the given root
                   V=variance(opt->variance,m,B,opt->c,opt->seqLength);                   
                    ...
                    output(opt->variance,result,tree1,tree2,tree3,m,y,rho,P,Suc1,Suc2,B,T,Labels,Support,phi);
                }
                    ...
                    double lambda;
                    if (opt->rate==NULL){//the rate is not given                        
                        if (strcmp(opt->estimate_root,"l")==0){//improve locally the root                            
                        ...
                        else{ //forget the root and re-estimate the position of the root over all branhces
                        ...
                        output(opt->variance,result,tree1,tree2,tree3,m,y,rho,P_new,Suc1_new,Suc2_new,B_new,T,Labels,Support_new,phi);
                    }
                    else{ //the rate is given
                        rho=readdouble(gr,(string)opt->rate);
                        if (strcmp(opt->estimate_root,"l")==0){//improve locally the root                            
                        ...
                        else{ //forget the root and re-estimate the position of the root over all branches                            
                ...
                if (opt->estimate_root==NULL){//keep the root
                    ...
                    if (opt->rate==NULL){//the rate is not given
                        ...
                        else  if (strcmp(opt->estimate_root,"a")==0){//forget the root and re-estimate the position of the root over all branhces                              
        ...
        ////////////////////////////unrooted tree////////////////////////////////////////
        else{ //unrooted tree, estimate the root
            ...
            if (!opt->constraint){//LD
                if (opt->rate==NULL){//the rate is not given              
                    ...
                    output(opt->variance,result,tree1,tree2,tree3,m+1,y,rho,P_new,Suc1_new,Suc2_new,B_new,T_new,Labels_new,Support_new,phi);
                }
                else {//le taux est donne, on n'estime que les dates
                    rho=readdouble(gr,(string)opt->rate);
                ...
                if (opt->rate==NULL){//the rate is not given
                   if (strcmp(opt->estimate_root,"a")==0){
                     ...
                     if (strcmp(opt->estimate_root,"a")==0){
                        ...
                        reroot_unrootedtree(m,r,lambda,P,B,T,P_new,B_new,T_new);
                        //cout<<"Improving the root position with constraints around the pre-estimated root ..."<<endl;
                        computeSuc(P_new,Suc1_new,Suc2_new,m+2,n+1);
                        cout<<"The branches are re-enumarated."<<endl;
