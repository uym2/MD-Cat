...
options* getInterface(bool & rooted)
{
    ...
    if (string(opt->inDateFile).length()==0){        
       opt->mrca=0;
       opt->leaves=1;
       cout<<"There is no date file, so you must provide the root date and the tips date (root date < tips date) for relative dates estimation."<<endl;                                              
       do {
           if (opt->leaves <= opt->mrca) cout<<"Root date must be smaller than tips date."<<endl;
...
void printInterface( FILE* in, options* opt , bool rooted)
{
	...
	if (opt->relative==true)
       fprintf(in,"  D                         Estimate relative dates : mrca date = %.3f, tip date =%.3f\n",opt->mrca,opt->leaves);
	else
...
void printHelp( void )
{
    ...
    FLAT"\t   searching the root on all branches. Moreover, when the constraint mode is chosen (option -c), method \"a\" firstly\n"
    FLAT"\t   estimates the root without using the constraints. After that, it uses the constraints to improve locally the position\n"
    FLAT"\t   of the root around this pre-estimated root. To use constraint mode on all branches in this case, please specify \"as\".\n"
    FLAT"\t   If the tree is not rooted, then the program searches the root on all branches. Similarly for the previous case, if\n"
    FLAT"\t   the constraint mode is chosen, method \"a\" uses only constraint mode to improve the root position around the pre-estimated\n"
	...
	switch( letter )
	{
		...
		case 'D':
             ...
             if (string(opt->inDateFile).length()==0){        
                ...
                do {
                    if (opt->leaves <= opt->mrca) cout<<"Root date must be smaller than tips date."<<endl;
                   opt->mrca = getInputReal("Ennter the root date (default=0)> ");
                   opt->leaves = getInputReal("Enter the tips date (default=1)> ");          
                }
                while (opt->leaves <= opt->mrca);
                opt->relative=true;
